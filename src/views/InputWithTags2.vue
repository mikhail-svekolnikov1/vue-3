  ...
  <div>
    <div class="container">
      <textarea
        ...
        @keydown="onKeyDown"
      />
      <div ref="menu" id="menu" class="menu" role="listbox"></div>
    </div>
  </div>
</template>
...
class Mentionify {
  ...
  onInput(ev) {
    ...
    setTimeout(() => {
      ...
      this.top =
        window.scrollY + coords.top + top + coords.height - this.ref.scrollTop;
...
const users = [{ username: "john doe" }, { username: "jane_doe" }];
...
onBeforeUnmount(() => {
  if (mentionify) {
    mentionify.ref.removeEventListener("input", mentionify.onInput);
    mentionify.ref.removeEventListener("keydown", mentionify.onKeyDown);
  }
});
</script>
